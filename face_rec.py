...
try:
    face_db = pd.read_csv("./face_reg_rec/face_database.csv", index_col=0)
                ...
                try:
                    ...
                    for fe in final_data["ENC"]:
                        if fr.compare_faces(np.array(eval(fe)), face_enc)[0]:
                            cv2.putText(img, final_data["NAMES"][final_data["ENC"].index(fe)].title(), (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,255,0), 2)
